...
from util import DRL_Agent, Net_AP
import numpy as np
"""
...
class DRL_Agent(object):
    ...
    def add2replay_memory(self, PDP_state_seq, PAS_state_seq, CSL_state_seq, action_seq, reward_seq, next_PDP_state_seq,
                          next_PAS_state_seq, next_CSL_state_seq, IS_weights_seq):
        if self.cache_length == 1:
            ...
            for i in range(len(input_data['PDP_state_seq'])):
                # add sample sequence to the replay buffer
                self.memory.append(
                    (PDP_state_seq[i], PAS_state_seq[i], CSL_state_seq[i], int(action_seq[i]), reward_seq[i],
                     next_PDP_state_seq[i], next_PAS_state_seq[i], next_CSL_state_seq[i], IS_weights_seq[i]))
        self.memory_len = len(self.memory)
    ...
    def train_DQN(self):
        ...
        IS_weights = np.zeros((num_samples, 1))
        ...
        if num_samples == 1:
            ...
            IS_weights[0, :] = replay_samples[-1][8]
        else:
            for i in range(num_samples):
                ...
                IS_weights[i, :] = replay_samples[i][8]
        ...
        IS_weights_th = torch.FloatTensor(IS_weights)
        """
        Forward Tensor to Device
        """
        ...
        IS_weights_th = IS_weights_th.to(self.device)
        """
        ...
        with torch.no_grad():
            q_next = self.target_model(next_PDP_state_th, next_PAS_state_th, next_CSL_state_th).gather(1,
                                                                                                       next_action_batch)
            # [batch_size, 1]
        q_val_train = reward_th + self.gamma * q_next
        TD_error = (q_val - q_val_train.detach()).to(self.device)
        # Weight TD errors
        weighted_TD_error = torch.mul(TD_error, IS_weights_th)
        zero_tensor = torch.zeros(weighted_TD_error.shape).to(self.device)
        abs_td_errors = np.abs(TD_error.detach().cpu().numpy())
        """
        Gradient Descent
        """
        self.model.train()
        with torch.enable_grad():
            optimizer.zero_grad()
            loss = loss_func(weighted_TD_error, zero_tensor)
            loss.backward()
        ...
        return loss.item(), abs_td_errors

    def update_target_DQN(self, tau=None):
...
def input_from_matlab(episode_, cache_length_, PDP_state_size_, PAS_state_size_, CSL_state_size_, action_size_,
                      ...
                      next_PDP_state_seq_, next_PAS_state_seq_, next_CSL_state_seq_, IS_weights_seq_):
    data = dict()
    ...
    # array.tolist()：将数组转换为具有相同元素的列表（list）
    data['IS_weights_seq'] = IS_weights_seq_.tolist()
...
if __name__ == "__main__":
    ...
    input_data = input_from_matlab(episode_from_matlab, cache_length_from_matlab,
                                   ...
                                   next_CSL_state_seq_from_matlab, IS_weights_seq_from_matlab)

    DNN_model_save_dir = r"{}/DQN_models".format(input_data['root_path'])
    mkdir(DNN_model_save_dir)
    Action_DNN_model_save_path = os.path.join(DNN_model_save_dir, 'DQN_AP_model.pt')
    DNN_memory_file_path = os.path.join(DNN_model_save_dir, 'replay_memory_AP.pkl')
    if_hard_update = True
    if input_data['operation_mode'] == 0:  # Training mode
        ...
        agent.model = Net_AP(agent.PDP_size, agent.PAS_size, agent.CSL_size, agent.action_size).to(agent.device)
        agent.target_model = Net_AP(agent.PDP_size, agent.PAS_size, agent.CSL_size, agent.action_size).to(agent.device)
        """
            parameters
        """
        # Root File Path of simulation file
        # Samples_Path.txt文件路径
        target_DNN_model_save_path = os.path.join(DNN_model_save_dir, 'target_DQN_AP_model.pt')
        Loss_record_file_path = os.path.join(DNN_model_save_dir, 'Loss_record_AP.pkl')
        Reward_record_file_path = os.path.join(DNN_model_save_dir, 'Reward_record_AP.pkl')
        train_loss_path = os.path.join(DNN_model_save_dir, 'DRL_AP_train_loss.pkl')
        ...
        agent.add2replay_memory(input_data['PDP_state_seq'], input_data['PAS_state_seq'], input_data['CSL_state_seq'],
                                ...
                                input_data['next_CSL_state_seq'], input_data['IS_weights_seq'])

        if os.path.exists(Reward_record_file_path):
            agent.load_reward_record(Reward_record_file_path)

        if agent.cache_length == 1:
            epoch_idx = (input_data['episode'] - 1) * agent.cache_length + 1
            agent.add2reward_record(epoch_idx, input_data['reward_seq'])
        else:
            for i in range(len(input_data['PDP_state_seq'])):
                epoch_idx = (input_data['episode'] - 1) * agent.cache_length + 1 + i
                agent.add2reward_record(epoch_idx, input_data['reward_seq'][i])
        ...
        if agent.memory_len >= 1:
            ...
            LOSS, abs_td_errors_for_update_SUMTREE = agent.train_DQN()
            ...
            """
            if if_hard_update:
                if agent.current_learning_step % 200 == 1:
                    agent.update_target_DQN(tau=1.0)
            ...
            abs_td_errors_for_update_SUMTREE = 0
        ...
        output_for_MATLAB['abs_TD_error'] = abs_td_errors_for_update_SUMTREE

    else:  # In action mode
        print('In action mode of AP')
        agent = DRL_Agent(input_data['PDP_state_size'], input_data['PAS_state_size'], input_data['CSL_state_size'],
                          input_data['action_size'], input_data['cache_length'], 1)
        agent.model = Net_AP(agent.PDP_size, agent.PAS_size, agent.CSL_size, agent.action_size).to(agent.device)
        if os.path.exists(Action_DNN_model_save_path):
            print('load AP DNN model')
            agent.load_model(Action_DNN_model_save_path)
        else:
            print('no AP DNN model')

        # Memory File
        if os.path.exists(DNN_memory_file_path):
            agent.load_memory(DNN_memory_file_path)

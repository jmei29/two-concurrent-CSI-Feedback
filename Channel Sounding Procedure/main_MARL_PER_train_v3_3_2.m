...
learning_step = 1;
snap_idx = 1;
total_simulated_CH_num = 4000;
learning_Step_Max = total_simulated_CH_num-1000;
%% Validation Section
if_perform_validate = 0;
return_val_index = 1;
validation_sample_num = 0;
return_val_discount_record = zeros(learning_Step_Max, 1);
return_val_discount_record_x_axis = zeros(learning_Step_Max, 1);
...
ep_idx = 1;
while 1
    if ep_idx < floor(parameters.num_episode_per_CASE*0.9)
        ep_idx = ep_idx + 1;
    else
        ep_idx = 1;
    end
    ...
    while 1
        ...
        if snap_idx > parameters.snap_total_num
            break;
        end
        ...
        if if_load_file
            ...
            file_open_path = strcat(parameters.CSI_data_saving_path, openfile);
        ...
        %% -----------------------Validation Process------------------------
        if mod(learning_step, 5) == 1 && CH_sounding_idx >= 3 + cache_length_from_matlab_to_python ...
                && snap_idx <= parameters.snap_total_num - 20 ...
                && validation_sample_num == 0
            if_perform_validate = 1;
        end
        if if_perform_validate
            if validation_sample_num >= 10 || ...
                    snap_idx + feedback_interval_in_snap > parameters.snap_total_num
                % end of validation process
                if validation_sample_num >= 10
                    return_val_discount_record_x_axis(return_val_index, 1) = ...
                        learning_step;
                    return_val_index = return_val_index + 1;
                end
                if validation_sample_num < 10
                    return_val_discount_record(return_val_index, 1) = 0;
                    Q_est_from_STA_side_versus_return(return_val_index, 1) = 0;
                    Q_est_from_AP_side_versus_return(return_val_index, 1) = 0;
                end
                validation_sample_num = 0;
                if_perform_validate = 0;
            else
        ...
        %% ----------------------------- Training --------------------------
        if CH_sounding_idx >= 3 + cache_length_from_matlab_to_python ...
                && if_train_DRL && ~if_perform_validate
            %% Training of DQN at STA side
            fprintf('Snap_idx = %d: Start Training of DQN at STA side. \n', snap_idx);
            [STA_b_idx, STA_sample_priorities, STA_samples_index_array, STA_IS_weights] ...
                = Memory_sample(cache_length_from_matlab_to_python, 'STA');
            STA_Sample_p_weights_record(learning_step, 1:length(STA_samples_index_array)) = ...
                STA_sample_priorities';
            train_result_for_STA = pyrunfile('MARL/MARL_STA_PER_train_v2.py', "output_for_MATLAB",...
                episode_from_matlab = learning_step,...
                cache_length_from_matlab = cache_length_from_matlab_to_python,...
                PDP_state_size_from_matlab = parameters.N_c,...
                PAS_state_size_from_matlab = parameters.AP_antenna_num, ...
                CSL_state_size_from_matlab = parameters.STA_antenna_num,...
                action_size_from_matlab = 8, ...
                Root_Path_File_from_matlab = DNN_file_save_dir,...
                Operation_mode_from_matlab = 0,... % training mode
                PDP_state_seq_from_matlab = CF_record(STA_samples_index_array,...
                1 : parameters.N_c), ...
                PAS_state_seq_from_matlab = CF_record(STA_samples_index_array,...
                parameters.N_c + 1 : parameters.N_c + parameters.AP_antenna_num), ...
                CSL_state_seq_from_matlab = CF_record(STA_samples_index_array,...
                parameters.N_c + parameters.AP_antenna_num + 1 : end), ...
                action_seq_from_matlab = action_record_STA_side(STA_samples_index_array, 1), ...
                reward_seq_from_matlab = reward_fun_record_STA_side(STA_samples_index_array, 1), ...
                next_PDP_state_seq_from_matlab = CF_record(STA_samples_index_array + 1, 1 : parameters.N_c), ...
                next_PAS_state_seq_from_matlab = CF_record(STA_samples_index_array + 1,...
                parameters.N_c + 1 : parameters.N_c + parameters.AP_antenna_num), ...
                next_CSL_state_seq_from_matlab = CF_record(STA_samples_index_array + 1,...
                parameters.N_c + parameters.AP_antenna_num + 1 : end),...
                IS_weights_seq_from_matlab = STA_IS_weights);
            train_result_for_STA = struct(train_result_for_STA);
            loss = double(train_result_for_STA.loss);
            LOSS_record_STA_side(max(learning_step,1), 1) = loss;
            learning_rate_record_STA_side(max(learning_step,1), 1) =...
                double(train_result_for_STA.current_learning_rate);
            abs_td_errors_for_update_SUMTREE = ...
                double(train_result_for_STA.abs_TD_error);
            Memory_batch_update(STA_b_idx, ...
                abs_td_errors_for_update_SUMTREE, 'STA');
            %% Training of DQN at AP side
            fprintf('Snap_idx = %d: Start Training of DQN at AP side. \n', snap_idx);
            [AP_b_idx, AP_sample_priorities, AP_samples_index_array, AP_IS_weights] = ...
                Memory_sample(cache_length_from_matlab_to_python, 'AP');
            AP_Sample_p_weights_record(learning_step, 1:length(AP_samples_index_array)) = ...
                AP_sample_priorities';
            train_result_for_AP = pyrunfile('MARL/MARL_AP_PER_train_v2.py',...
                "output_for_MATLAB", episode_from_matlab = learning_step,...
                cache_length_from_matlab = cache_length_from_matlab_to_python,...
                PDP_state_size_from_matlab = parameters.N_c,...
                PAS_state_size_from_matlab = parameters.AP_antenna_num, ...
                CSL_state_size_from_matlab = parameters.STA_antenna_num,...
                ...
                Root_Path_File_from_matlab = DNN_file_save_dir,...
                Operation_mode_from_matlab = 0,... % training mode
                PDP_state_seq_from_matlab = CFI_record(AP_samples_index_array,...
                1:parameters.N_c), ...
                PAS_state_seq_from_matlab = CFI_record(AP_samples_index_array,...
                parameters.N_c + 1 : parameters.N_c + parameters.AP_antenna_num), ...
                CSL_state_seq_from_matlab = CFI_record(AP_samples_index_array,...
                parameters.N_c + parameters.AP_antenna_num + 1 : end), ...
                action_seq_from_matlab = action_record_AP_side(AP_samples_index_array, 1), ...
                reward_seq_from_matlab = reward_fun_record_AP_side(AP_samples_index_array, 1), ...
                next_PDP_state_seq_from_matlab = CFI_record(AP_samples_index_array + 1, 1:parameters.N_c), ...
                next_PAS_state_seq_from_matlab = CFI_record(AP_samples_index_array + 1,...
                parameters.N_c + 1 : parameters.N_c + parameters.AP_antenna_num), ...
                next_CSL_state_seq_from_matlab = CFI_record(AP_samples_index_array + 1,...
                parameters.N_c + parameters.AP_antenna_num + 1 : end),...
                IS_weights_seq_from_matlab = AP_IS_weights);
            train_result_for_AP = struct(train_result_for_AP);
            loss = double(train_result_for_AP.loss);
            LOSS_record_AP_side(max(learning_step,1), 1) = loss;
            learning_rate_record_AP_side(max(learning_step,1), 1) = ...
                double(train_result_for_AP.current_learning_rate);
            abs_td_errors_for_update_SUMTREE = ...
                double(train_result_for_AP.abs_TD_error);
            Memory_batch_update(AP_b_idx, ...
                abs_td_errors_for_update_SUMTREE, 'AP');
            %% Renew learning step
            learning_step = double(train_result_for_STA.current_learning_step);
    ...
    %% save files
    if mod(CH_sounding_idx, 10) == 0
        savefile = sprintf('/Case_%d_CSI_fed_result_DQN_training_CASE_%d.mat', parameters.CASE_IDX, training_CASE);
        ...
        save(file_save_path, ...
            ...
            'return_val_discount_record','return_val_discount_record_x_axis',...
